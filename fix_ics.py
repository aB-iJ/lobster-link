...
# Redoing the ICS generation with a format that Apple Calendar prefers
lines = [
    "BEGIN:VCALENDAR",
    "VERSION:2.0",
...
def add_event(title, wday, start_time, end_time, location, weeks_list):
    ...
    for w in weeks_list:
        ...
        lines.extend([
            "BEGIN:VEVENT",
            f"UID:openclaw-schedule-v2-{uid_counter}@localhost",
            f"DTSTAMP:{now_str}Z",
...
with open('schedule2.ics', 'w', encoding='utf-8') as f:
    # Use CRLF which is standard for iCalendar
    f.write('\r\n'.join(lines))
